...
function diaBounds(dia: string) {
  return {
    ini: new Date(`${dia}T00:00:00`).toISOString(),
    fim: new Date(`${dia}T23:59:59`).toISOString(),
  };
}
...
function horaMinDeISO(iso: string): string {
  const d = new Date(iso);
...
const S = {
  btnRose: {
    ...
    borderRadius: "10px", fontWeight: 700, cursor: "pointer", fontFamily: FONT_BODY,
  } as React.CSSProperties,
  btnOut: {
    background: C.white, color: C.text2, border: `1.5px solid ${C.roseMid}`,
    borderRadius: "10px", fontWeight: 600, cursor: "pointer", fontFamily: FONT_BODY,
  } as React.CSSProperties,
  mini: {
    borderRadius: "8px", padding: "5px 11px",
    fontSize: "0.78rem", fontWeight: 600, cursor: "pointer", border: "none", fontFamily: FONT_BODY,
  } as React.CSSProperties,
  inp: {
    width: "100%", background: C.bgAlt,
    border: `1.5px solid ${C.roseMid}`, borderRadius: "10px",
    color: C.text, fontSize: "0.9rem", padding: "0.65rem 0.85rem",
    outline: "none", boxSizing: "border-box" as const,
    fontFamily: FONT_BODY, colorScheme: "light" as const, marginBottom: "0.2rem",
  } as React.CSSProperties,
  lbl: {
    color: C.text2, fontSize: "0.7rem", letterSpacing: "0.1em",
    textTransform: "uppercase" as const, marginBottom: "5px",
    display: "block", marginTop: "0.75rem", fontFamily: FONT_BODY,
  } as React.CSSProperties,
  card: {
    background: C.white, border: `1px solid ${C.roseMid}`,
    borderRadius: "12px", padding: "0.9rem 1.1rem",
    display: "flex", alignItems: "center", gap: "1rem",
    flexWrap: "wrap" as const, boxShadow: `0 2px 12px rgba(236,168,169,0.1)`,
  } as React.CSSProperties,
  secTitle: {
    fontFamily: FONT_TITLE, color: C.text, fontWeight: 700,
    fontSize: "1.3rem", margin: "0 0 1.25rem", letterSpacing: "-0.01em",
  } as React.CSSProperties,
};
...
export default function AdminPage() {
  ...
  const [walkinHora,   setWalkinHora]   = useState<string>("");
  ...
  const fetchCitas = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .gte("fecha_hora", ini).lte("fecha_hora", fim)
      .order("fecha_hora");
    if (data) setCitas(data as unknown as CitaRow[]);
  }, [diaVer]);

  // ── Fetch faturamento hoy ──
  const fetchFatHoje = useCallback(async () => {
    const { ini, fim } = diaBounds(hojeISO());
    const { data } = await supabase
      .from("citas").select("servicios(precio)")
      .eq("estado", "completada")
      .gte("fecha_hora", ini).lte("fecha_hora", fim);
    if (data) setFatHoje(data.reduce((acc: number, c: any) => acc + (c.servicios?.precio ?? 0), 0));
  }, []);

  // ── Fetch faturamento semana ──
  const fetchFatSemana = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq("estado", "completada")
      .gte("fecha_hora", ini).lte("fecha_hora", fim);
    if (data) setFatSemana(data.reduce((acc: number, c: any) => acc + (c.servicios?.precio ?? 0), 0));
  }, []);

  // ── Fetch faturamento mês ──
  const fetchFatMes = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq("estado", "completada")
      .gte("fecha_hora", ini).lte("fecha_hora", fim);
    if (data) setFatMes(data.reduce((acc: number, c: any) => acc + (c.servicios?.precio ?? 0), 0));
  }, []);

  const fetchClientes = useCallback(async () => {
    const { data } = await supabase
      .from("clientes").select("id, nombre, telefono, citas_asistidas, citas_perdidas").order("nombre");
    if (data) setClientes(data);
  }, []);

  const fetchServicios = useCallback(async () => {
    const { data } = await supabase
      .from("servicios").select("id, nombre, precio, activo, duracion_minutos").order("nombre");
    if (data) {
      setServicios(data);
  ...
  async function guardarWalkin() {
    ...
    const { data: ex } = await supabase.from("clientes").select("id")
      .eq("telefono", telefone || nome).single();
    if (ex) {
      cliente_id = ex.id;
    } else {
      const { data: nv } = await supabase.from("clientes")
        .insert({ nombre: nome, telefono: telefone || null }).select("id").single();
      if (!nv) return;
      cliente_id = nv.id;
    }
    await supabase.from("citas").insert({
      cliente_id, servicio_id: servico_id,
      fecha_hora: new Date(`${diaVer}T${hora}:00`).toISOString(),
      estado: "confirmada", notas: "Walk-in",
    });
  ...
  if (!auth) return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: FONT_BODY }}>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.roseMid}`, borderRadius: "24px", padding: "3rem 2.5rem", width: "100%", maxWidth: "380px", textAlign: "center", boxShadow: `0 12px 50px rgba(236,168,169,0.18)` }}>
        ...
        <input
          type="password"
          style={{ ...S.inp, textAlign: "center", marginBottom: "0.75rem" }}
          placeholder="••••••••"
          value={pwd}
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "1.75rem 1rem" }}>
        ...
        <div
          ...
          style={{
            ...
            boxShadow: `0 2px 12px rgba(236,168,169,0.1)`,
            overflowX: "auto",
            flexWrap: "nowrap",
          ...
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              style={{
                background: tab === t.key ? C.rose : "transparent",
                color: tab === t.key ? C.white : C.text2,
                ...
                : citasAtivas.slice(0, 6).map(c => {
                    ...
                    return (
                      <div key={c.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 0", borderBottom: `1px solid ${C.roseMid}`, flexWrap: "wrap" }}>
                        ...
                        <span style={{ color: C.roseDeep, fontSize: "0.8rem", fontWeight: 600 }}>{c.servicios?.precio ?? "—"} €</span>
        ...
        {tab === "calendario" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
              ...
              <input
                type="date" value={diaVer}
                onChange={e => setDiaVer(e.target.value)}
                style={{ ...S.inp, width: "auto", padding: "0.45rem 0.8rem", fontSize: "0.85rem" }}
              />
              ...
              {HORAS_AGENDA.map((hora, idx) => {
                ...
                return (
                  <div key={hora}
                    style={{
                      display: "flex",
                      minHeight: vazia ? "58px" : "auto",
                      borderBottom: idx < HORAS_AGENDA.length - 1 ? `1px solid ${C.roseMid}` : "none",
                    }}>
                    {/* Coluna hora */}
                    <div style={{
                      ...
                      paddingTop: "14px",
                      borderRight: `1px solid ${C.roseMid}`,
                      background: C.bgAlt,
                    }}>
                    ...
                    <div style={{ flex: 1, padding: vazia ? "0" : "0.6rem 0.75rem", display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                      {vazia ? (
                        <div
                          ...
                          onClick={() => abrirWalkin(hora)}
                          style={{
                            flex: 1, display: "flex", alignItems: "center",
                            paddingLeft: "1rem",
                            color: C.text3, fontSize: "0.78rem",
                          ...
                          return (
                            <div key={c.id}
                              ...
                              <div style={{ display: "flex", gap: "0.35rem" }}>
                                ...
                                {c.clientes?.telefono && (
  <a
    href={`https://wa.me/+${c.clientes.telefono.replace(/\D/g,"")}?text=Olá%20${encodeURIComponent(c.clientes.nombre??"")}!%20Lembramos%20a%20sua%20marcação%20às%20${horaCita}%20na%20Glow%20Esthetic%20✦`}
              ...
              {clientes.length === 0
                ...
                : clientes.map(cl => (
                    <div key={cl.id} style={S.card}>
                      ...
                      {cl.telefono && (
  <a
    href={`https://wa.me/+${cl.telefono.replace(/\D/g,"")}?text=Olá%20${encodeURIComponent(cl.nombre)}!%20Não%20se%20esqueça%20da%20sua%20próxima%20marcação%20na%20Glow%20Esthetic%20✦`}
    target="_blank" rel="noreferrer"
    ...
    💬 WhatsApp
              ...
              {servicios.map(sv => (
                <div key={sv.id} style={{ ...S.card, opacity: sv.activo ? 1 : 0.5 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <input
                      type="number"
                      value={editPreco[sv.id] ?? sv.precio}
                  ...
                  <button
                    onClick={() => toggleServico(sv.id, sv.activo)}
      ...
      {modalWalkin && (
        <div
          style={{ position: "fixed", inset: 0, background: "rgba(51,51,51,0.5)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}
          ...
          <div style={{ background: C.white, border: `1px solid ${C.roseMid}`, borderRadius: "22px", padding: "2.25rem", width: "100%", maxWidth: "400px", position: "relative", boxShadow: "0 24px 70px rgba(51,51,51,0.18)" }}>
            <button
              onClick={() => { setModalWalkin(false); setWalkinHora(""); }}
              style={{ position: "absolute", top: "1.25rem", right: "1.25rem", background: C.roseLight, border: "none", color: C.roseDark, cursor: "pointer", borderRadius: "50%", width: "30px", height: "30px", fontSize: "0.9rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
            ...
            <input
              ...
              style={S.inp}
              value={walkin.hora}
              onChange={e => setWalkin(w => ({ ...w, hora: e.target.value }))}
            />

            <button
              style={{ ...S.btnRose, width: "100%", padding: "0.85rem", marginTop: "1.1rem", fontSize: "0.95rem", boxShadow: `0 6px 20px rgba(236,168,169,0.35)` }}
              onClick={guardarWalkin}>
              Guardar Marcação
            </button>
          </div>
        </div>
